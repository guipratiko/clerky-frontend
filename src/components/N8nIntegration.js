...
const TabPanel = ({ children, value, index, ...other }) => (
  <div role="tabpanel" hidden={value !== index} id={`integration-tabpanel-${index}`} aria-labelledby={`integration-tab-${index}`} {...other}>
...
const N8nIntegration = () => {
  ...
  const handleToggle = async (integrationId, isActive) => {
    try {
      ...
      toast.success(
        t('n8nIntegration.integrationToggled', {
          status: isActive ? t('n8nIntegration.activated') : t('n8nIntegration.deactivated')
        })
      );
      loadData();
    } catch (error) {
                  ...
                  <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={2}>
                    ...
                    <Box display="flex" gap={1}>
                      <Tooltip title={integration.isActive ? t('n8nIntegration.deactivate') : t('n8nIntegration.activate')}>
                        <IconButton size="small" onClick={() => handleToggle(integration._id, !integration.isActive)}>
                      ...
                      <Tooltip title={t('n8nIntegration.test')}>
                        ...
                        >
                          {testingIntegration === integration._id ? <CircularProgress size={20} /> : <TestIcon />}
                        </IconButton>
                      </Tooltip>
                      <Tooltip title={t('n8nIntegration.delete')}>
                        <IconButton size="small" onClick={() => handleDelete(integration._id)} color="error">
                          <DeleteIcon />
                        </IconButton>
                      </Tooltip>
                    </Box>
                  ...
                  >
                    ...
                    {instances.map((instance) => (
                      <MenuItem key={instance.instanceName} value={instance.instanceName}>
                        {instance.displayLabel || instance.displayName || instance.instanceName} ({instance.status})
                      </MenuItem>
                    ))}
                  </Select>
                </FormControl>
